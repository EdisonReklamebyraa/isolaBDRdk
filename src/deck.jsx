...
class Deck extends React.Component {
    ...
    _updateURL(slide,suffix){
        suffix = suffix || "";
        slide = (this.props.children.length)?slide:"";
        this.context.router.transitionTo( this._getBase() + slide + suffix);       
    }
    
    _toggleOverviewMode() {
        this._updateURL(this.context.slide ,this.context.overview ? "" : "?overview")
    }
    
    _togglePresenterMode() {
        this._updateURL(this.context.slide ,this.context.presenter ? "" : "?presenter")
    }
    ...
    _goToSlide(e) {
        if (e.key === "spectacle-slide") {
            ...
            if (this._checkFragments(this.context.slide, data.forward)) {
                this._updateURL(this.context.slide ,this._getSuffix())
    ...
    _prevSlide() {
        ...
        if ((this._checkFragments(this.context.slide, false) || this.context.overview) && this.props.children.length) {
            ...
            this._updateURL(this._getHash(slide) , this._getSuffix());
        }
        localStorage.setItem("spectacle-slide",
    ...
    _nextSlide() {
        ...
        if ((this._checkFragments(this.context.slide, true) || this.context.overview) && this.props.children.length) {
    ...
    _getHash(slide) {
        let hash = slide;
        if ("id" in this._getChild(slide).props) {
            hash = this._getChild(slide).props.id;
        }
        return hash;
    }
    ...
    _getChild(slide){
        ...
        return this.props.children[slide] || this.props.children
    }
    ...
    getActiveSlide(){
        return this.props.children[this._getSlideIndex()];
    }
...
Deck.propTypes = {
    ...
    progress: React.PropTypes.oneOf(["pacman", "bar", "tabs","number","gallery","none"])
};
